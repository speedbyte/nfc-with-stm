 ...
 910  02b4 84            	pop	a
 912  02b5               L343:
 913                     ; 344   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
 ...
 937  02d9               L743:
 938                     ; 349 	I2C_SendData(M24LR04E_I2C,(uint8_t)ReadAddr); // LSB 
 ...
 945  02e2 84            	pop	a
 947  02e3               L353:
 948                     ; 352   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
 ...
 972  0307               L753:
 973                     ; 358   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
 975  0307 4b01          	push	#1
 976  0309 ae5210        	ldw	x,#21008
 977  030c cd0000        	call	_I2C_GenerateSTART
 979  030f 84            	pop	a
 981  0310               L363:
 982                     ; 361   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT)&& I2C_TimeOut-->0)  // EV5 
...
1006  0334               L763:
1007                     ; 366   I2C_Send7bitAddress(M24LR04E_I2C, EE_address, I2C_Direction_Receiver);
...
1015  033f 85            	popw	x
1017  0340               L373:
1018                     ; 369   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)&& I2C_TimeOut-->0)  // EV6 
...
1042  0364               L304:
1043                     ; 374   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_RECEIVED)&& I2C_TimeOut-->0)  // EV7 
...
1120  03c9 81            	ret
1187                     ; 399 void M24LR04E_WriteOneByte (uint8_t EE_address, uint16_t ReadAddr, uint8_t pBuffer)
1187                     ; 400 {
1188                     	switch	.text
...
1235  0404               L164:
1236                     ; 412   I2C_Send7bitAddress(M24LR04E_I2C, EE_address, I2C_Direction_Transmitter);
...
1244  040f 85            	popw	x
1246  0410               L564:
1247                     ; 415   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)&& I2C_TimeOut-->0) /* EV6 */
...
1271  0434               L174:
1272                     ; 420 	I2C_SendData(M24LR04E_I2C,(uint8_t)(ReadAddr >> 8)); // MSB 
...
1279  043d 84            	pop	a
1281  043e               L574:
1282                     ; 423   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
...
1306  0462               L105:
1307                     ; 428 	I2C_SendData(M24LR04E_I2C,(uint8_t)ReadAddr); // LSB 
...
1314  046b 84            	pop	a
1316  046c               L505:
1317                     ; 430   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
...
1341  0490               L115:
1342                     ; 435   I2C_SendData(M24LR04E_I2C, pBuffer);
...
1349  0499 84            	pop	a
1351  049a               L515:
1352                     ; 438   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) /* EV8 */
...
1376  04be               L125:
1377                     ; 443   I2C_GenerateSTOP(M24LR04E_I2C, ENABLE);
...
1388  04c9 81            	ret
1468                     ; 459 void M24LR04E_ReadBuffer (uint8_t EE_address, uint16_t ReadAddr, uint8_t NumByteToRead,uint8_t* pBuffer)
1468                     ; 460 {
1469                     	switch	.text
...
1523  050d               L765:
1524                     ; 477   I2C_Send7bitAddress(M24LR04E_I2C, EE_address , I2C_Direction_Transmitter);
...
1532  0518 85            	popw	x
1534  0519               L375:
1535                     ; 480   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)&& I2C_TimeOut-->0) // EV6 
...
1559  053d               L775:
1560                     ; 485 	I2C_SendData(M24LR04E_I2C,(uint8_t)(ReadAddr >> 8)); // MSB 
...
1567  0546 84            	pop	a
1569  0547               L306:
1570                     ; 490   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
...
1594  056b               L706:
1595                     ; 495 	I2C_SendData(M24LR04E_I2C,(uint8_t)ReadAddr); // LSB 
...
1602  0574 84            	pop	a
1604  0575               L316:
1605                     ; 498   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
...
1629  0599               L716:
1630                     ; 504   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
1632  0599 4b01          	push	#1
1633  059b ae5210        	ldw	x,#21008
1634  059e cd0000        	call	_I2C_GenerateSTART
1636  05a1 84            	pop	a
1638  05a2               L326:
1639                     ; 507   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT)&& I2C_TimeOut-->0)  // EV5 
...
1663  05c6               L726:
1664                     ; 512   I2C_Send7bitAddress(M24LR04E_I2C, EE_address, I2C_Direction_Receiver);
...
1672  05d1 85            	popw	x
1674  05d2               L336:
1675                     ; 515   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)&& I2C_TimeOut-->0)  // EV6 
...
1700  05f8               L156:
1701                     ; 525 		while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_RECEIVED)&& I2C_TimeOut-->0)  
...
1888                     ; 583   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Transmitter);
...
1895  06ac 85            	popw	x
1897  06ad               L327:
1898                     ; 586   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) /* EV6 */
...
1908                     ; 591   I2C_SendData(M24LR04E_I2C, M24LR04E_REG_TEMP);
1910  06bb 4b00          	push	#0
1911  06bd ae5210        	ldw	x,#21008
1912  06c0 cd0000        	call	_I2C_SendData
1914  06c3 84            	pop	a
1916  06c4               L137:
1917                     ; 594   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
1927                     ; 600   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
1929  06d2 4b01          	push	#1
1930  06d4 ae5210        	ldw	x,#21008
1931  06d7 cd0000        	call	_I2C_GenerateSTART
1933  06da 84            	pop	a
1935  06db               L737:
1936                     ; 603   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT))  /* EV5 */
...
1946                     ; 608   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Receiver);
...
1953  06f3 85            	popw	x
1955  06f4               L547:
1956                     ; 611   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED))  /* EV6 */
...
1967  0702               L357:
1968                     ; 616   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_RECEIVED))  /* EV7 */
...
1978                     ; 621   RegValue = I2C_ReceiveData(M24LR04E_I2C) << 8;
...
2014                     ; 634   RegValue |= I2C_ReceiveData(M24LR04E_I2C);
...
2117                     ; 661   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Transmitter);
...
2124  0783 85            	popw	x
2126  0784               L3101:
2127                     ; 664   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) /* EV6 */
...
2137                     ; 669   I2C_SendData(M24LR04E_I2C, M24LR04E_REG_CONF);
2139  0792 4b01          	push	#1
2140  0794 ae5210        	ldw	x,#21008
2141  0797 cd0000        	call	_I2C_SendData
2143  079a 84            	pop	a
2145  079b               L1201:
2146                     ; 672   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
2156                     ; 678   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
2158  07a9 4b01          	push	#1
2159  07ab ae5210        	ldw	x,#21008
2160  07ae cd0000        	call	_I2C_GenerateSTART
2162  07b1 84            	pop	a
2164  07b2               L7201:
2165                     ; 681   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT))  /* EV5 */
...
2175                     ; 686   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Receiver);
...
2182  07ca 85            	popw	x
2184  07cb               L5301:
2185                     ; 689   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED))  /* EV6 */
...
2300                     ; 727   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Transmitter);
...
2307  0828 85            	popw	x
2309  0829               L5701:
2310                     ; 730   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) /* EV6 */
...
2320                     ; 735   I2C_SendData(M24LR04E_I2C, M24LR04E_REG_CONF);
2322  0837 4b01          	push	#1
2323  0839 ae5210        	ldw	x,#21008
2324  083c cd0000        	call	_I2C_SendData
2326  083f 84            	pop	a
2328  0840               L3011:
2329                     ; 738   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
2339                     ; 743   I2C_SendData(M24LR04E_I2C, RegValue);
...
2346  0857 84            	pop	a
2348  0858               L1111:
2349                     ; 746   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
2359                     ; 751   I2C_GenerateSTOP(M24LR04E_I2C, ENABLE);
...
2369  086f 84            	pop	a
2370  0870 81            	ret
2398                     ; 759 static void M24LR04E_LowLevel_DeInit(void)
2398                     ; 760 {
2399                     	switch	.text
2400  0871               L3_M24LR04E_LowLevel_DeInit:
2404                     ; 762   I2C_Cmd(M24LR04E_I2C, DISABLE);
2406  0871 4b00          	push	#0
2407  0873 ae5210        	ldw	x,#21008
2408  0876 cd0000        	call	_I2C_Cmd
2410  0879 84            	pop	a
2411                     ; 764   I2C_DeInit(M24LR04E_I2C);
2413  087a ae5210        	ldw	x,#21008
2414  087d cd0000        	call	_I2C_DeInit
2416                     ; 767   CLK_PeripheralClockConfig(M24LR04E_I2C_CLK, DISABLE);
2418  0880 ae0300        	ldw	x,#768
2419  0883 cd0000        	call	_CLK_PeripheralClockConfig
2421                     ; 770   GPIO_Init(M24LR04E_I2C_SCL_GPIO_PORT, M24LR04E_I2C_SCL_PIN, GPIO_Mode_In_PU_No_IT);
2423  0886 4b40          	push	#64
2424  0888 4b02          	push	#2
2425  088a ae500a        	ldw	x,#20490
2426  088d cd0000        	call	_GPIO_Init
2428  0890 85            	popw	x
2429                     ; 773   GPIO_Init(M24LR04E_I2C_SDA_GPIO_PORT, M24LR04E_I2C_SDA_PIN, GPIO_Mode_In_PU_No_IT);
...
2437                     ; 775 }
2440  089c 81            	ret
2465                     ; 782 static void M24LR04E_LowLevel_Init(void)
2465                     ; 783 {
2466                     	switch	.text
2467  089d               L5_M24LR04E_LowLevel_Init:
2471                     ; 785   CLK_PeripheralClockConfig(M24LR04E_I2C_CLK, ENABLE);
